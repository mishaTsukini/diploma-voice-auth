    ...
    const handleSubmit = async (e) => {
        ...
        await axios({
            ...
            .then((response) => {
                ...
                if (response.success) {
                    setNotification({
                        message: `Пользватель ${user} успешно зарегистрирован!`,
                        type: "sucess"
                    })
                    resetForm()
                    fetchUserData()
                    ...
                    if (response.code[0] === Configs.ERROR_CODE.EXISTED_USER) {
                        setNotification({
                            message: `Пользователь ${user} уже существует`,
                            type: "danger"
                        })
                        ...
                        setNotification({
                            message: `Не удалось зарегистрировать пользователя ${user} ! Убедитесь в корректности данных`,
                            type: "sucess"
                        })
                        resetForm()
                        fetchUserData()
            ...
            <Row>
                <Col md={8}>
                    ...
                    <Form>
                        ...
                        <Form.Group as={Row}>
                            <Col sm="12">
                                <Button onClick={handleSubmit} variant="primary">Зарегистрироваться</Button>
                            </Col>
                        </Form.Group>
                    </Form>
                </Col>
                ...
                >
                    <Modal.Body>
                        <div className="text-center">
                            ...
                            <p className="mt-2">Подождите, пока данные обработаются</p>
                        </div>
                    </Modal.Body>
                </Modal>
            )}
